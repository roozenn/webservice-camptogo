...
def get_order_detail(order_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    order = db.query(Order).options(joinedload(Order.items).joinedload(OrderItem.product).joinedload(Product.images), joinedload(Order.address), joinedload(Order.timeline)).filter(Order.id == order_id, Order.user_id == user.id).first()
    if not order:
    ...
    timeline = [TimelineItem(status=t.status, description=t.description, created_at=t.created_at) for t in order.timeline]
    data = OrderDetailResponseData(
        id=order.id,
